...
const Dashboard: React.FC = () => {
  ...
  return <Layout>
      <div className="space-y-6">
        ...
        {widgets.length > 0 && <div className="space-y-6">
            <div className="flex items-center justify-between">
        ...
        {(loading || loadingData) && <div className="flex items-center justify-center py-12">
        ...
        <WidgetConfigModal open={showWidgetConfig} onOpenChange={setShowWidgetConfig} onSave={handleAddWidget} />
      </div>
    </Layout>;
};
  ...
  return <Card className={`group relative ${getGridSpan()} shadow-elegant hover:shadow-glow transition-all duration-300 border-l-4 border-l-primary`}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" className="opacity-0 group-hover:opacity-100 transition-opacity h-6 w-6 p-0" onClick={onRemove}>
            ×
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        ...
        {widget.type === 'chart' && <div className="space-y-3">
            {widget.app === 'google-analytics' ? <>
              ...
              width: `${widget.data.sessions / (widget.data.sessions + widget.data.users) * 100}%`
            }}></div>
                </div>
              </> : <>
                <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div className="w-full bg-secondary h-3 rounded-full overflow-hidden">
                  <div className="bg-gradient-primary h-3 rounded-full transition-all duration-500" style={{
              width: `${widget.data.completed / (widget.data.completed + widget.data.pending) * 100}%`
            }}></div>
                </div>
              </>}
          </div>}
        
        {widget.type === 'table' && <div className="space-y-3">
            {Array.isArray(widget.data) && widget.data.slice(0, 4).map((row: any, index: number) => <div key={index} className="flex justify-between items-center text-sm p-2 rounded-lg hover:bg-accent/50 transition-colors">
                <span className="font-medium">{row.customer || row.metric}</span>
                <div className="flex items-center gap-2">
                  <span className="font-semibold">{row.amount || row.value}</span>
                  {row.change && <Badge variant={row.change.startsWith('+') ? 'default' : 'destructive'} className="text-xs">
                      {row.change}
                    </Badge>}
                </div>
              </div>)}
